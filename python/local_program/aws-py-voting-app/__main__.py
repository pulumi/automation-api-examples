...
# The ECS cluster in which our application and database will run
...
lambda_policy_attachment = aws.iam.RolePolicyAttachment("app-lambda-policy", role=app_task_role.name,
...
redis_targetgroup = aws.alb.TargetGroup("redis-targetgroup",
                                        port=redis_port,
                                        protocol="TCP",
                                        target_type="ip",
                                        vpc_id=app_vpc.id)
...
redis_balancer = aws.alb.LoadBalancer("redis-balancer",
...
redis_listener = aws.alb.Listener("redis-listener",
...
redis_task_definition = aws.ecs.TaskDefinition("redis-task-definition",
                                               ...
                                               container_definitions=json.dumps([{
                                                   ...
                                                   # A pre-built docker image with a functioning redis server
                                                   "image": "redis:alpine",
                                                   "memory": 512,
                                                   "essential": True,
                                                   "portMappings": [{
...
flask_targetgroup = aws.alb.TargetGroup("flask-targetgroup",
                                        port=80,
                                        protocol="TCP",
                                        target_type="ip",
                                        vpc_id=app_vpc.id)
...
# Creating a load balancer to spread out incoming requests
flask_balancer = aws.alb.LoadBalancer("flask-balancer",
                                      load_balancer_type="network",
                                      internal=False,
                                      security_groups=[],
                                      subnets=[app_vpc_subnet.id])
...
flask_listener = aws.alb.Listener("flask-listener",
